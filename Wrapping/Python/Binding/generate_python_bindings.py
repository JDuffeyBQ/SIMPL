...
def generate_simpl_bindings(output_dir: str, header_path: str, body_path: str, files: List[str], source_dir: str, python_output_dir: str, plugin_name: str, body_top_path: str = '', no_tests: bool = False, relative_imports: bool = False, python_test_output_dir: str = '') -> None:
  module_target_file_path = f'{output_dir}/py_simpl.cpp'
  module_temp_file_path = f'{module_target_file_path}.temp'
  if not python_test_output_dir:
    python_test_output_dir = python_output_dir
  with open(module_temp_file_path, 'w') as module_file, open(f'{python_output_dir}/simplpy.py', 'w') as python_file, ExitStack() as exit_stack:
    if not no_tests:
      test_file = exit_stack.enter_context(open(f'{python_test_output_dir}/simpl_UnitTest.py', 'w'))
    ...
    if not no_tests:
      if relative_imports:
        test_file.write(f'from dream3d import simpl\n\n')
      else:
...
def generate_plugin_bindings(output_dir: str, module_name: str, files: List[str], include_dir: Optional[str], python_output_dir: str, plugin_name: str, header_path: str = '', body_path: str = '', body_top_path: str = '', no_tests: bool = False, relative_imports: bool = False, python_test_output_dir: str = '') -> None:
  ...
  with open(module_temp_file_path, 'w') as module_file, open(f'{python_output_dir}/{module_name}py.py', 'w') as python_file, ExitStack() as exit_stack:
    if not no_tests:
      test_file = exit_stack.enter_context(open(f'{python_test_output_dir}/{module_name}_UnitTest.py', 'w'))
    includes = files
    if include_dir:
      includes = []
      for file_path in files:
    ...
    if not no_tests:
      if relative_imports:
        test_file.write(f'from dream3d import {module_name}\n\n')
      else:
...
if __name__ == '__main__':
  ...
  parser.add_argument('--python_test_output_dir')
  ...
  if args.plugin:
    ...
    generate_plugin_bindings(args.output_dir, args.module_name, files, args.include_dir, args.python_output_dir, args.plugin_name, args.header_path, args.body_path, args.body_top_path, args.no_tests, args.relative_imports, args.python_test_output_dir)
  else:
    ...
    generate_simpl_bindings(args.output_dir, args.header_path, args.body_path, files, args.source_dir, args.python_output_dir, args.plugin_name, args.body_top_path, args.no_tests, args.relative_imports, args.python_test_output_dir)
